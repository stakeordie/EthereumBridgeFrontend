...
export class Exchange extends StoreConstructor {
  ...
  @computed
  get step() {
    return this.stepsConf[this.stepNumber];
  }
  ...
  setStatus() {
    ...
    console.log('this.operation.token', this.operation.token)
    console.log('this.operation.amount', this.operation.amount)
    console.log('this.operation.fee', this.operation.fee)
    console.log('this.operation.type', this.operation.type)
    console.log('this.operation.ethAddress', this.operation.ethAddress)
    console.log('this.operation.oneAddress', this.operation.oneAddress)
  }
  ...
  async approveEcr20() {
    ...
    let params = {
      id: uuid(),
      transactionHash: '',
      mode: EXCHANGE_MODE.APPROVE_ERC20,
      type: TOKEN.ERC20,
      token: this.transaction.tokenSelected ? this.transaction.tokenSelected.symbol : '',
      fee: this.ethNetworkFee,
      erc20Address: this.transaction.erc20Address,
      from: this.transaction.erc20Address,
      to: this.transaction.erc20Address,
    };

    await this.createOperation(params);
    ...
    contract.ethMethodsERC20.callApprove(
      ...
      this.stores.userMetamask.erc20TokenDetails.decimals, async (result) => {
        ...
        if (result.receipt) {
          this.operation.confirmations = result.confirmation
  ...
  async swapSnip20ToEth(isEth: boolean) {
    ...
    this.operation.status = await this.updateOperation(
      this.operation.id,
      {
        transactionHash: Snip20SwapHash({
          tx_id,
